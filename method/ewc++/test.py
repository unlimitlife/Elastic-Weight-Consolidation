...
def test(net, test_loader, config, data_config):
    ...
    single_total_accuracy_per_task = [(idx, single_class_accuracy[cls:cls+classes_per_task].mean()) for idx, cls in enumerate(range(0,cml_classes,classes_per_task))]
    for (idx, acc) in single_total_accuracy_per_task:
        log.info('\n[Accuracy of Task%s] - single-head: %.3lf%%' %
                 (str(idx), acc))
